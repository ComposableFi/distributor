...
pub struct RemoveNewClaim<'info> {
...
pub fn handle_admin_remove_new_claim(
    ...
    let total_back_amount = amount_locked.checked_add(amount_unlocked).ok_or(ErrorCode::ArithmeticError)?;
    ...
    let distributor = &mut ctx.accounts.distributor;
    distributor.max_total_claim =  distributor.max_total_claim.checked_sub(total_back_amount).ok_or(ErrorCode::ArithmeticError)?;
    ...
    msg!(
        "Removed new claim with locked {} and {} unlocked with lockup start:{} end:{}",
        claim_status.locked_amount,
        claim_status.unlocked_amount,
        distributor.start_ts,
        distributor.end_ts,
    );
    emit!(RemoveNewClaimEvent {
